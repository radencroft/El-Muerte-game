...
public class ShadowInventory : MonoBehaviour
{
    ...
    {
        // CAN SHOOT? 
        if (weapon_id != 0) { GetComponent<ShadowClone>().stopShooting = false; }
        else { GetComponent<ShadowClone>().stopShooting = true; }
    }
    ...
    void SwitchWeapon()
    {
        if (Input.GetButtonDown("ChangeWeapon") && !GetComponent<ShadowClone>().stopMovement)
        {
